...
fn create_and_write(path: &std::path::Path, generated: String) {
    let mut file = match std::fs::File::create(path) {
        Err(why) => panic!("elitetobash: couldn't write to {}: {}", path.display(), why),
        Ok(file) => file
    };
...
fn main() {
    ...
    if cli_arguments.len() < 2 {
        println!("elitetobash - create bash scripts from elite\n\
                 ----------\n\
    ...
    let y = parse::elite_bash::parse(x);
    let z = format!("{}_out.sh", cli_arguments.get(1).unwrap()).as_str().to_owned();
